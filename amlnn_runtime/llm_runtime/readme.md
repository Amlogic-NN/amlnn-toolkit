...
`llm_runtime` provides the C/C++ inference SDK for deploying **large language models** on Amlogic NPU platforms. Link against `libllmsdk.so` and include `llmsdk.h` to integrate LLM inference into native applications. Supports Android and Linux (Buildroot / Yocto / Debian) platforms.
